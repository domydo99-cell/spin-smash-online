...
- 2026-03-21: 操作しやすさ改善（滑り軽減 + ハンドルアシスト）。
  - タイトル/ロビーに `ハンドルアシスト` の ON/OFF トグルを追加。
  - 入力ベクトルへターゲット追従の軽い補正を追加（逆方向入力時は補正しない）。
  - 人間プレイヤーのみ慣性をわずかに抑え、曲がりやすさと減速感を向上。
  - キャッシュ更新: `duel.*` を `20260321-13`、SWを `spin-smash-shell-v19` に更新。
- 2026-03-21: 起動時チュートリアル/ルール導線を追加。
  - 起動直後にスキップ可能な説明モーダル（`チュートリアル` / `ルール説明` タブ）を自動表示。
  - 対戦画面右上に `説明` ボタンを追加し、いつでも同モーダルを再表示できるように変更。
  - モーダルは `Esc / Enter / Space` でも閉じられるように対応。
  - キャッシュ更新: `duel.*` を `20260321-14`、SWを `spin-smash-shell-v20` に更新。
- 2026-03-21: 検証。
  - `node --check public/duel.js` OK
  - `npm run build` OK
